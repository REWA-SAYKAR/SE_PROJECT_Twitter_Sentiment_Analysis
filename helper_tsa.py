...
def preprocessing_data(word_query, number_of_tweets, function_option):
  ...
  data["hashtags"] = data["Tweets"].apply(extract_hastag)
...
def analyse_hastag(data):
  ...
  hastag = pd.DataFrame(data["hashtags"].to_list()).add_prefix("hashtag_")

  try:
    hastag = pd.concat([hastag["hashtag_0"], hastag["hashtag_1"], hastag["hashtag_2"]], ignore_index=True)
  except:
    hastag = pd.concat([hastag["hashtag_0"]], ignore_index=True)
